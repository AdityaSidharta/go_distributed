...
func InitMapReduce(nmap int, nreduce int,
	...
	mr := new(MapReduce)
	mr.nMap = nmap       // Number of Map jobs
	mr.nReduce = nreduce // Number of Reduce jobs
	mr.file = file
...
// Register assign Worker to the Master.registerChannel
...
// Shutdown Removes Listener and set Alive status to False
...
// StartRegistrationServer setup the server to accept requests through unix domain socket
...
func (mr *MapReduce) Split(fileName string) {
	...
	defer infile.Close()
	fi, err := infile.Stat() // FileInfo structure describing file
	if err != nil {
		log.Fatal("Split: ", err)
	}
	size := fi.Size()
	fmt.Printf("Split read %d\n", size)
	nchunk := size / int64(mr.nMap) // determine size per chunk
	nchunk += 1                     // Ensure No Chunk with empty sizes
	...
	m := 1 // index for filename
	i := 0 // Current line number in infile
...
func DoMap(JobNumber int, fileName string,
	...
	for r := 0; r < nreduce; r++ {
		...
		enc := json.NewEncoder(file)                   // write to JSON
		for e := res.Front(); e != nil; e = e.Next() { // Front returns the First Element of the List
			kv := e.Value.(KeyValue) // How does List become KeyValue?
...
func DoReduce(job int, fileName string, nmap int,
	...
	for i := 0; i < nmap; i++ {
		...
		for {
			var kv KeyValue
			err = dec.Decode(&kv) // decode per Key Value
			if err != nil {
				break
			}
			_, ok := kvs[kv.Key]
			if !ok {
				kvs[kv.Key] = list.New() // Create New List for all new encountered Key
			}
	...
	sort.Strings(keys) // Reduce Sort Key, and Perform the operation in increasing order
...
func (mr *MapReduce) Merge() {
	...
	for i := 0; i < mr.nReduce; i++ {
		...
		fmt.Printf("Merge: read %s\n", p) // Read Input File, output from Reduce
		file, err := os.Open(p)
		if err != nil {
			log.Fatal("Merge: ", err)
		}
		...
		for {
			...
			kvs[kv.Key] = kv.Value // After reduce function, key should be unique
		}
		file.Close()
	}
	var keys []string
...
// CleanupRegistration ShutDown the Master Process
